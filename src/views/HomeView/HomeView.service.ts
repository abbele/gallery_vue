...
export class HomeViewService {
  static getImgs = (limit: string): Promise<ImgListModel> =>
    ApiService.get<IImgResponse[]>(API_IMG.GET_LIST, undefined, { limit }).then(
      (res) => new ImgListModel(res),
    )
  ...
  static getQuotes = (limit: string): Promise<QuotesModel> => {
    ...
    return ApiService.get<IQuotesResponse[]>(API_QUOTE.GET_LIST, headers, {
      limit,
    }).then((res) => new QuotesModel(res))
  }
}
